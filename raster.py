...
from ._prediction import dpl_predict_output, predict_output, predict_prob, predict_multioutput, torch_regression_output_scaler, torch_regression_multioutput_scaler, dpl_predict_multioutput, mlp_predict_output, uns_predict_output, uns_predict_multioutput, torch_predict_output, torch_predict_multioutput
...
class Raster(RasterPlot, BaseRaster):
    ...
    def tst_predict(self, estimator, file_path=None, in_memory=False,
        driver="GTiff", dtype=None, nodata=None, n_jobs=1,
        progress=False, **kwargs):
        """Apply prediction of a scikit learn model to a Raster.
    ...
    def uns_predict(self, estimator, file_path=None, in_memory=False,
                driver="GTiff", dtype=None, nodata=None, n_jobs=1,
                progress=False, **kwargs):
        """Apply prediction of a scikit learn model to a Raster.
        ...
        # chose prediction function
        if len(indexes) == 1:
            predfun = partial(uns_predict_output, estimator=estimator)
        else:
            predfun = partial(uns_predict_multioutput, estimator=estimator)
    ...
    def dpl_predict(self, estimator, shape, file_path=None, in_memory=False,
                driver="GTiff", dtype=None, nodata=None, n_jobs=1,
                progress=False, **kwargs):
        """Apply prediction of a scikit learn model to a Raster.
        ...
        # chose prediction function
        if len(indexes) == 1:
            predfun = partial(dpl_predict_output, estimator=estimator, shape=shape)
        else:
            predfun = partial(dpl_predict_multioutput, estimator=estimator, shape=shape)
    ...
    def torch_dpl_predict(self, estimator, shape, file_path=None, in_memory=False,
                driver="GTiff", dtype=None, nodata=None, n_jobs=1,
                progress=False, **kwargs):
        """Apply prediction of a scikit learn model to a Raster.
        ...
        # chose prediction function
        if len(indexes) == 1:
            predfun = partial(torch_predict_output, estimator=estimator, shape=shape)
        else:
            predfun = partial(torch_predict_multioutput, estimator=estimator, shape=shape)
    ...
    def torch_regression_dpl_output_scaler(self, estimator, shape, scaler, target_meta, file_path=None, in_memory=False,
                driver="GTiff", dtype=None, nodata=None, n_jobs=1,
                progress=False, **kwargs):
        tfile = None
        ...
        if result.ndim > 1:
            n_outputs = result.shape[1]  # Ensure correct dimension
        else:
            n_outputs = 1
        ...
        if len(indexes) == 1:
            predfun = partial(torch_regression_output_scaler, estimator=estimator, shape=shape, scaler=scaler)
        else:
            predfun = partial(torch_regression_multioutput_scaler, estimator=estimator, shape=shape, scaler=scaler)
        ...
        if target_meta is None:
            ...
            meta = target_meta
            meta.update(count=n_outputs)  # Ensure correct number of bands
            original_meta = self.meta.copy()
            #count = len(indexes)
            meta.update(height=original_meta["height"], width=original_meta["width"], transform=original_meta["transform"])
            #meta.update(kwargs)
        windows = [w for w in self.block_shapes(*self.block_shape)]
        data_gen = ((w, self.read(window=w, masked=True)) for w in windows)
        ...
        prefix = "pred_raw_"
        names = [prefix + str(i) for i in range(n_outputs)]  # Updated to n_outputs
        new_raster = self._copy(output_dst, names)
    ...
    def mlp_predict(self, estimator, file_path=None, in_memory=False,
                driver="GTiff", dtype=None, nodata=None, n_jobs=1,
                progress=False, **kwargs):
        """Apply prediction of a scikit learn model to a Raster.
        ...
        # chose prediction function
        if len(indexes) == 1:
            predfun = partial(mlp_predict_output, estimator=estimator)
        else:
            predfun = partial(mlp_predict_multioutput, estimator=estimator)
    ...
    def to_crs(self, crs, resampling="nearest", file_path=None, in_memory=False,
        ...
        if in_memory is False:
            with rasterio.open(file_path, "w", driver=driver, SRC_METHOD=NO_GEOTRANSFORM, **meta) as dst:
                for i, layer in enumerate(self.iloc):
                    reproject(
